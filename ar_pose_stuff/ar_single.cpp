...
//     ROS_INFO ("\tMarker Pattern Filename: %s", pattern_filename_);
    ...
    // **** advertsie 
	centrePub_ = n.advertise<std_msgs::Int32MultiArray>("ar_marker_image_pos", 1);
  ...
  void ARSinglePublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
	...
	topc = lefc = rigc = botc = 0;
	for(i=0; i < 4; i++)
	{
		if(marker_info[k].vertex[i][0] < marker_info[k].pos[0]) lefc += marker_info[k].vertex[i][0];
		else rigc += marker_info[k].vertex[i][0];
		if(marker_info[k].vertex[i][1] < marker_info[k].pos[1]) topc += marker_info[k].vertex[i][1];
		else botc += marker_info[k].vertex[i][1];
	}
	 copy(marker_info[k].pos, marker_info[k].pos+2, centrepoint_.data.begin());
	 centrepoint_.data[2] = (rigc-lefc)/2; centrepoint_.data[3] = (botc-topc)/2;
      ...
      // **** publish the marker
		...
		centrePub_.publish(centrepoint_);
